...
WINDOW_NAME = "Lightweight Drowsiness Detection"
...
BUTTON_X1, BUTTON_Y1 = 350, 310
BUTTON_X2, BUTTON_Y2 = 470, 350
...
def draw_calibration_ui(frame, elapsed):
    cv2.rectangle(frame, (10, 10), (470, 90), (0, 0, 0), -1)

    if elapsed < INIT_STEP_TIME:
        remain = INIT_STEP_TIME - elapsed
        message = "초기화 1단계: 눈을 뜨고 입을 닫아주세요"
        sub = f"{remain:.1f}초 후 다음 단계"
    else:
        remain = INIT_STEP_TIME * 2 - elapsed
        message = "초기화 2단계: 눈을 감고 입을 닫아주세요"
        sub = f"{remain:.1f}초 후 완료"

    frame = put_korean_text(frame, message, (20, 25), 20, (0, 255, 255))
    frame = put_korean_text(frame, sub, (20, 55), 18, (255, 255, 255))
...
def draw_reset_button(frame):
    ...
    frame = put_korean_text(frame, "초기화", (BUTTON_X1 + 25, BUTTON_Y1 + 8), 20, (255, 255, 255))
    return frame
...
while True:
    ...
    if result.multi_face_landmarks:
        ...
        cv2.putText(frame, f"EAR: {ear:.2f}", (15, 105),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

        cv2.putText(frame, f"MAR: {mar:.2f}", (15, 130),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

        cv2.putText(frame, f"EAR TH: {personal_ear_threshold:.2f}", (15, 155),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

        cv2.putText(frame, f"MAR TH: {personal_mar_threshold:.2f}", (15, 180),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

        cv2.putText(frame, f"Eye Prob: {eye_closed_prob:.2f}", (15, 205),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

        cv2.putText(frame, f"Yawn Prob: {yawn_prob:.2f}", (15, 230),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

        cv2.putText(frame, f"Fusion: {fusion_score:.2f}", (15, 255),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

        cv2.putText(frame, f"Count: {drowsy_count}", (15, 280),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    ...
    if calibration_mode:
        ...
        cv2.rectangle(frame, (10, 290), (470, 340), (0, 0, 255), -1)

        cv2.putText(frame, alert_message, (20, 323),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)

    elif status == "NORMAL":
        cv2.putText(frame, "NORMAL", (15, 55),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)

    else:
        cv2.putText(frame, "NO FACE", (15, 55),
